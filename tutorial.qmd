...
When working in RStudio, these updates are easily obtained. Error detection can be addressed by including code to print error explanations within functions. For example, we may want to limit the type of objects given to a function. In the following example, we only accept a scalar input.
...
# ... code with cat() updates ...

# Stop logging
sink()
```

Additionally, Alpine can send you emails saying if your script has stopped because of an error (or completion) but it does not give details.

All of the above is straightforward, but it it requires that you consistently pull up RStudio or log into servers. You have to always have access to your computer to check errors/progress, along with signing into VPNs if your server has such securities.
...
will send notifications every minute until you go into the app and recognize the notification.

## Example Workflow

In the following example, we show how `pushoverr` can be used to display errors and progress.
